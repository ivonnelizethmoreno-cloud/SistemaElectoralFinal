...
class VotoControllerTest {
    ...
    void setUp() {
        ...
        authOrdinario = new UsernamePasswordAuthenticationToken(
                usuarioOrdinario.getUsername(),
                usuarioOrdinario.getPassword()
        );
    }
    ...
    void muestraSelectorParaVotanteIndigena() {
        ...
        Authentication authIndigena =
                new UsernamePasswordAuthenticationToken("ana", "pwd");

        when(userAccountRepository.findByUsername("ana"))
                .thenReturn(usuarioIndigena);

        Model model = new ExtendedModelMap();
        RedirectAttributes redirect = new RedirectAttributesModelMap();

        String vista = controller.mostrarTarjeton(authIndigena, null, model, redirect);
    ...
    void bloqueaVistaSiUsuarioYaVoto() {
        ...
        when(userAccountRepository.findByUsername("carlos"))
                .thenReturn(usuarioOrdinario);

        Model model = new ExtendedModelMap();
        RedirectAttributes redirect = new RedirectAttributesModelMap();

        String vista = controller.mostrarTarjeton(authOrdinario, "ordinario", model, redirect);

        assertThat(vista).isEqualTo("bloqueo-post-voto");
        assertThat(model.getAttribute("mensaje"))
                .isEqualTo("Señor(a) Ciudadano(a) usted ya votó, sólo se permite un intento");
    ...
    void filtraTarjetonSegunCircunscripcionElegida() {
        Partido partidoOrdinario = Partido.builder()
                .partidoId(1L).circunscripcion("ORDINARIA").build();

        Partido partidoIndigena = Partido.builder()
                .partidoId(2L).circunscripcion("INDIGENA").build();

        Candidato candidatoOrdinario = Candidato.builder()
                .cedula(10L).circunscripcion("ORDINARIA").build();

        Candidato candidatoIndigena = Candidato.builder()
                .cedula(20L).circunscripcion("INDIGENA").build();
        ...
        when(perteneceRepository.findAllConPartidoYCandidato())
                .thenReturn(List.of(ordinario, indigena));
    ...
    void registraVotoPreferenteYMarcaComoVotado() {
        ...
        Candidato candidato = Candidato.builder()
                .cedula(999L).nombre("Candidato Preferente")
                .circunscripcion("ORDINARIA")
                .build();

        when(candidatoRepository.findById(999L))
                .thenReturn(Optional.of(candidato));

        when(eligeRepository.save(any(Elige.class)))
                .thenAnswer(inv -> inv.getArgument(0));

        when(userAccountRepository.save(usuarioOrdinario))
                .thenReturn(usuarioOrdinario);
    ...
    void registraVotoNoPreferenteTomandoPrimerCandidato() {
        ...
        Partido partido = Partido.builder()
                .partidoId(15L).circunscripcion("ORDINARIA").tipoLista("cerrada")
                .build();

        Candidato primero = Candidato.builder()
                .cedula(111L).circunscripcion("ORDINARIA")
                .build();
        ...
        when(perteneceRepository.findByPartido_PartidoIdOrderByOrdenCandidatosAsc(15L))
                .thenReturn(List.of(relacion));

        when(candidatoRepository.findById(111L))
